...
local function SwitchOff(f)
	...
	if not f.player then
		...
		if f.friend then
			f.name:SetTextColor(0.0, 1.0, 0.0) -- Green for friendly NPCs
		else
			...
			if r and g and b and r > 0.9 and g == 0 and b == 0 then
				...
				f.name:SetTextColor(1.0, 0.0, 0.0) -- Red for hostile NPCs
			else
				-- All other non-friendly NPCs are neutral (yellow)
				f.name:SetTextColor(1.0, 1.0, 0.0) -- Yellow for neutral NPCs
			end
		end
	end

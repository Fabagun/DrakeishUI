...
local function SetName(self)
	...
	-- Abbreviate name if it exceeds healthbar width (only for NPCs)
	if not self.player and self.health then
		local healthbarWidth = self.health:GetWidth()
		local nameText = self.name.text
		
		-- Set the name temporarily to measure its width
		self.name:SetText(nameText)
		local nameWidth = self.name:GetStringWidth()
		
		-- If name is wider than healthbar, truncate it
		if nameWidth > healthbarWidth then
			local utf8sub = kui.utf8sub
			local maxLength = string.len(nameText)
			...
			while minLen <= maxLen do
				local midLen = floor((minLen + maxLen) / 2)
				local testName = utf8sub(nameText, 1, midLen) .. "..."
				self.name:SetText(testName)
				local testWidth = self.name:GetStringWidth()
				
				if testWidth <= healthbarWidth then
					bestFit = testName
					minLen = midLen + 1
				else
					maxLen = midLen - 1
			...
			-- Use the best fit if found, otherwise use a very short version
			if bestFit ~= "" then
				self.name.text = bestFit
			else
				-- Fallback: use first few characters
				self.name.text = utf8sub(nameText, 1, 5) .. "..."
			end
		end
	end

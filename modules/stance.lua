...
function anchor:stancebar_update()
	...
	if leftbar and rightbar then
		self:SetPoint('TOPLEFT', pUiMainBar, 'TOPLEFT', offsetX, leftOffset-4);
	elseif leftbar then
		self:SetPoint('TOPLEFT', pUiMainBar, 'TOPLEFT', offsetX, rightOffset);
	elseif rightbar then
		self:SetPoint('TOPLEFT', pUiMainBar, 'TOPLEFT', offsetX, leftOffset-4);
	else
		self:SetPoint('TOPLEFT', pUiMainBar, 'TOPLEFT', offsetX, nobar-4);
	end
end
...
local function stancebutton_update()
	if not InCombatLockdown() then
		_G.ShapeshiftButton1:SetPoint('BOTTOMLEFT', anchor, 'BOTTOMLEFT', 2, 0)
	end
end
...
local function stancebutton_position()
	for index=1, NUM_SHAPESHIFT_SLOTS do
		...
		if index == 1 then
			button:SetPoint('BOTTOMLEFT', anchor, 'BOTTOMLEFT', 2, 0)
		else

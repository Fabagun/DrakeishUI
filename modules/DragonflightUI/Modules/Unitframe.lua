...
function Module:OnInitialize()
    ...
    -- Check if RegisterModuleOptions exists (WoTLK compatibility)
    if DF.RegisterModuleOptions then
...
function Module.CreatePlayerFrameTextures()
    local base = 'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\uiunitframe'
    ...
    if not frame.PlayerFrameBackground then
        ...
        background:SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-Player-PortraitOn-BACKGROUND'
        )
    ...
    if not frame.PlayerFrameBorder then
        ...
        border:SetTexture('Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-Player-PortraitOn-BORDER')
...
function Module.ChangeStatusIcons()
    ...
    -- Hide the PlayerFrameGroupIndicator completely
...
function Module.HookVertexColor()
    PlayerFrameHealthBar:HookScript(
        'OnValueChanged',
        function(self)
            -- Always use green color for player frame, no class colors
            PlayerFrameHealthBar:GetStatusBarTexture():SetTexture(
                'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-Health'
            )
            PlayerFrameHealthBar:SetStatusBarColor(0, 1, 0, 1) -- Green color
        end
    )
        ...
        function(self)
            if Module.db.profile.target.classcolor and UnitIsPlayer('target') then
                TargetFrameHealthBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Health-Status'
                )
                ...
                TargetFrameHealthBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Health'
                )
...
function Module.ChangePlayerframe()
    ...
    PlayerName:SetPoint('BOTTOMLEFT', PlayerFrameHealthBar, 'TOPLEFT', 0, 1 + 1)

    PlayerLevelText:ClearAllPoints()
    PlayerLevelText:SetPoint('BOTTOMRIGHT', PlayerFrameHealthBar, 'TOPRIGHT', -5, 1 + 1)
    ...
    if powerTypeString == 'MANA' then
        PlayerFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-Mana'
        )
    elseif powerTypeString == 'RAGE' then
        PlayerFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-Rage'
        )
    elseif powerTypeString == 'FOCUS' then
        PlayerFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-Focus'
        )
    elseif powerTypeString == 'ENERGY' then
        PlayerFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-Energy'
        )
    elseif powerTypeString == 'RUNIC_POWER' then
        PlayerFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOn-Bar-RunicPower'
        )
    end
    ...
    -- Texture coordinates will be set dynamically in UpdateStatus function
...
function Module.HookPlayerStatus()
    ...
    -- Hide all Blizzard glow textures to prevent interference
    PlayerStatusGlow:Hide()
    if PlayerFrameGlow then PlayerFrameGlow:Hide() end
    if PlayerFrameCombatGlow then PlayerFrameCombatGlow:Hide() end
    if PlayerFrameCombatIconGlow then PlayerFrameCombatIconGlow:Hide() end
    if PlayerRestGlow then PlayerRestGlow:Hide() end
    
    -- Hide additional Blizzard glow textures
    local additionalGlows = {
        'PlayerFrameFlash',
        'PlayerAttackIcon',
        'PlayerAttackBackground'
    }
    
    for _, glowName in ipairs(additionalGlows) do
        ...
        elseif IsResting() then
            ...
            PlayerStatusTexture:SetTexCoord(Module.GetCoords('UI-HUD-UnitFrame-Player-PortraitOn-Status'))
        ...
        elseif PlayerFrame.onHateList then
            ...
            PlayerStatusTexture:SetAlpha(1.0)
			PlayerStatusTexture:SetDrawLayer("OVERLAY", 15)
			PlayerStatusTexture:SetBlendMode("ADD")
        elseif PlayerFrame.inCombat then
            ...
            PlayerStatusTexture:Show()
            PlayerStatusTexture:SetVertexColor(1.0, 0, 0, 1.0)
            PlayerStatusTexture:SetAlpha(0.6)
			PlayerStatusTexture:SetDrawLayer("OVERLAY", 9)
			PlayerStatusTexture:SetBlendMode("ADD")
        else
            frame.PlayerFrameDeco:Show()
    ...
    -- Call UpdateStatus immediately to set initial texture coordinates
    UpdateStatus()
    
    -- Hook Blizzard glow textures to prevent them from showing
    if PlayerStatusGlow then
        hooksecurefunc(PlayerStatusGlow, 'Show', function() PlayerStatusGlow:Hide() end)
    end
    if PlayerFrameGlow then
        hooksecurefunc(PlayerFrameGlow, 'Show', function() PlayerFrameGlow:Hide() end)
    end
    if PlayerFrameCombatGlow then
        hooksecurefunc(PlayerFrameCombatGlow, 'Show', function() PlayerFrameCombatGlow:Hide() end)
    end
    if PlayerRestGlow then
        hooksecurefunc(PlayerRestGlow, 'Show', function() PlayerRestGlow:Hide() end)
    end
    
    -- Additional comprehensive hooks for any other Blizzard glow textures
    local additionalGlows = {
        'PlayerFrameFlash',
        'PlayerFrameCombatGlow', 
        'PlayerFrameCombatIconGlow',
        'PlayerAttackIcon',
        'PlayerAttackBackground'
    }
    ...
    for _, glowName in ipairs(additionalGlows) do
        local glow = _G[glowName]
        if glow then
            hooksecurefunc(glow, 'Show', function() glow:Hide() end)
            glow:Hide()
            glow:SetAlpha(0)
        end
    end
...
function Module.ChangeTargetFrame()
    ...
    if not frame.TargetFrameBackground then
        ...
        background:SetDrawLayer('BACKGROUND', 2)
        background:SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-Target-PortraitOn-BACKGROUND'
        )
    ...
    if not frame.TargetFrameBorder then
        ...
        border:SetDrawLayer('ARTWORK', 2)
        border:SetTexture('Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-Target-PortraitOn-BORDER')
    ...
    TargetFrameTextureFrameName:SetPoint('BOTTOM', TargetFrameHealthBar, 'TOP', 10, 3 - 2 + 1.5)

    TargetFrameTextureFrameLevelText:ClearAllPoints()
    TargetFrameTextureFrameLevelText:SetPoint('BOTTOMRIGHT', TargetFrameHealthBar, 'TOPLEFT', 16, 3 - 2 + 1.5)
    ...
    -- Set color to transparent and prevent changes
    TargetFrameNameBackground:SetVertexColor(1, 1, 1, 0)
    TargetFrameNameBackground.SetVertexColor = function(self, r, g, b, a)
        -- Do nothing - keep the original texture color
    end
    
    -- Hook the original target frame update to get the correct reaction colors
    hooksecurefunc("TargetFrame_Update", function()
        if UnitExists("target") then
            local r, g, b = UnitSelectionColor("target")
            TargetFrameTextureFrameName:SetTextColor(r, g, b, 1)
            
            -- Apply standard WoW level difficulty colors to level text
            local playerLevel = UnitLevel("player")
            local targetLevel = UnitLevel("target")
            
            if targetLevel and playerLevel then
                -- Use GetQuestDifficultyColor for standard WoW difficulty colors
                local difficultyColor = GetQuestDifficultyColor(targetLevel, playerLevel)
                TargetFrameTextureFrameLevelText:SetTextColor(difficultyColor.r, difficultyColor.g, difficultyColor.b, 1)
            else
                -- Fallback to reaction color if level info is not available
                TargetFrameTextureFrameLevelText:SetTextColor(r, g, b, 1)
            end
        end
    end)
    ...
    if true then
        ...
        if not frame.TargetFrameFlash then
            ...
            flash:SetTexture(
                'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-InCombat'
            )
    ...
    if not frame.PortraitExtra then
        ...
        extra:SetTexture('Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\uiunitframeboss2x')
    ...
    -- Hook target frame buff updates to style buttons with border
    hooksecurefunc("TargetFrame_UpdateAuras", function()
        for i = 1, 32 do
            local buff = _G["TargetFrameBuff"..i]
            if buff then
                StyleTargetAuraIcon(buff)
            end
        end
    end)
end
...
function Module.ReApplyTargetFrame()
    ...
    if powerTypeString == 'MANA' then
        TargetFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Mana'
        )
    elseif powerTypeString == 'FOCUS' then
        TargetFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Focus'
        )
    elseif powerTypeString == 'RAGE' then
        TargetFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Rage'
        )
    elseif powerTypeString == 'ENERGY' then
        TargetFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-Energy'
        )
    elseif powerTypeString == 'RUNIC_POWER' then
        TargetFrameManaBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Target-PortraitOn-Bar-RunicPower'
        )
    end
...
function Module.ChangeToT()
    ...
    if not frame.TargetFrameToTBackground then
        ...
        background:SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-BACKGROUND'
        )
    ...
    if not frame.TargetFrameToTBorder then
        ...
        border:SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-BORDER'
        )
    ...
    if not frame.ToTManaBar then
        ...
        f:SetStatusBarTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-Mana'
        )
        ...
        local UpdateManaBarTextures = function()
            ...
            elseif powerTypeString == 'FOCUS' then
                frame.ToTManaBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-Focus'
                )
            elseif powerTypeString == 'RAGE' then
                frame.ToTManaBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-Rage'
                )
            elseif powerTypeString == 'ENERGY' then
                frame.ToTManaBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-Energy'
                )
            elseif powerTypeString == 'RUNIC_POWER' then
                frame.ToTManaBar:GetStatusBarTexture():SetTexture(
                    'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-RunicPower'
                )
            end
...
function Module.ReApplyToT()
    if UnitExists('playertargettarget') then
        ...
        TargetFrameToTHealthBar:GetStatusBarTexture():SetTexture(
            'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-TargetofTarget-PortraitOn-Bar-Health'
        )
...
function Module.ChangeFocusFrame()
    ...
    -- Set color to transparent and prevent changes
    FocusFrameNameBackground:SetVertexColor(1, 1, 1, 0)
    FocusFrameNameBackground.SetVertexColor = function(self, r, g, b, a)
        -- Do nothing - keep the original texture color
    end
    ...
    -- Set font sizes to match player and target frames
    FocusFrameTextureFrameName:SetFont("Fonts\\FRIZQT__.TTF", 10, "OUTLINE")
    FocusFrameTextureFrameLevelText:SetFont("Fonts\\FRIZQT__.TTF", 10, "OUTLINE")
    FocusFrameTextureFrameHealthBarText:SetFont("Fonts\\FRIZQT__.TTF", 9, "OUTLINE")
    FocusFrameTextureFrameManaBarText:SetFont("Fonts\\FRIZQT__.TTF", 9, "OUTLINE")
    ...
    --[[    FocusFrameHealthBar:GetStatusBarTexture():SetTexture(
        'Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Unitframe\\UI-HUD-UnitFrame-Player-PortraitOff-Bar-Health'
    )
    FocusFrameHealthBar:SetStatusBarColor(1, 1, 1, 1) ]]
    -- Mana 119,12
...
function Module.ChangePetFrame()
    ...
    PetFrame:SetPoint('TOPLEFT', PlayerFrame, 'TOPLEFT', 80, -100)
	...
	-- Hide original pet level text
    PetLevelText:Hide()
    
    -- Create custom pet level text
    if not frame.CustomPetLevelText then
        local customLevelText = PetFrame:CreateFontString('CustomPetLevelText', 'OVERLAY', 'TextStatusBarText')
        customLevelText:SetPoint('BOTTOMRIGHT', PetFrameHealthBar, 'TOPRIGHT', -7, 1 + 1)
        
        -- Match pet name styling
        local petNameFont, petNameSize, petNameFlags = PetName:GetFont()
        customLevelText:SetFont(petNameFont, petNameSize, petNameFlags)
        customLevelText:SetTextColor(PetName:GetTextColor())
        customLevelText:SetJustifyH("RIGHT")
        customLevelText:SetJustifyV("BOTTOM")
        
        -- Add shadow to match pet name
        customLevelText:SetShadowOffset(1, -1)
        customLevelText:SetShadowColor(0, 0, 0, 1)
        frame.CustomPetLevelText = customLevelText
    end
    
    -- Hook pet frame updates to update custom level text
    hooksecurefunc("PetFrame_Update", function()
        if UnitExists("pet") then
            local petLevel = UnitLevel("pet")
            if petLevel and petLevel > 0 then
                frame.CustomPetLevelText:SetText(petLevel)
                frame.CustomPetLevelText:Show()
            else
                frame.CustomPetLevelText:Hide()
            end
        else
            frame.CustomPetLevelText:Hide()
        end
...
function Module.CreateRestFlipbook()
    if not frame.RestIcon then
        if DF.Wrath then
            ...
            restTexture:SetTexture('Interface\\Addons\\DrakeishUI\\modules\\DragonflightUI\\Textures\\uiunitframerestingflipbook')

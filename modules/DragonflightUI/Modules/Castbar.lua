...
function Module.ChangeDefaultCastbar()
    ...
    -- Hide only the target castbar icon, keep the castbar visible
    if TargetFrameSpellBar then
        -- Hide all possible icon elements
        if TargetFrameSpellBar.Icon then
            TargetFrameSpellBar.Icon:Hide()
            TargetFrameSpellBar.Icon:SetAlpha(0)
        end
        if TargetFrameSpellBar.IconTexture then
            TargetFrameSpellBar.IconTexture:Hide()
            TargetFrameSpellBar.IconTexture:SetAlpha(0)
        end
        
        -- Hide all regions that might be icons
        local regions = {TargetFrameSpellBar:GetRegions()}
        for i, region in ipairs(regions) do
            if region.GetTexture and region:GetTexture() then
                local texture = region:GetTexture()
                if texture and (string.find(texture, "icon") or string.find(texture, "Icon") or region:GetWidth() == region:GetHeight()) then
                    region:Hide()
                    region:SetAlpha(0)
                end
            end
        ...
        -- Apply DragonflightUI textures to target castbar
        local standardRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarStandard2'
        local borderRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarFrame2'
        local backgroundRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarBackground2'
        local sparkRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarSpark'
        
        -- Hide original target castbar regions to avoid double frames
        local originalRegions = {TargetFrameSpellBar:GetRegions()}
        for i, region in ipairs(originalRegions) do
            if region.GetTexture and region:GetTexture() then
                region:Hide()
                region:SetAlpha(0)
            end
        ...
        -- Resize the target castbar (30% thinner and 20% shorter)
        TargetFrameSpellBar:SetSize(175, 8)
        
        -- Move the entire bar 20px to the left (15px + 5px)
        TargetFrameSpellBar:ClearAllPoints()
        TargetFrameSpellBar:SetPoint('TOPLEFT', TargetFrame, 'BOTTOMLEFT', 25 - 20, -23)
        
        -- Create background texture (grey texture under the filling one)
        local bgTex = TargetFrameSpellBar:CreateTexture('TargetCastbarBackground', 'BACKGROUND')
        bgTex:SetAllPoints()
        bgTex:SetTexture(backgroundRef)
        
        -- Set the status bar texture and make it visible
        if TargetFrameSpellBar:GetStatusBarTexture() then
            TargetFrameSpellBar:GetStatusBarTexture():SetTexture(standardRef)
            TargetFrameSpellBar:GetStatusBarTexture():SetAlpha(1)
            TargetFrameSpellBar:GetStatusBarTexture():Show()
            -- Make the progress bar wider on Y-axis to fill gaps
            TargetFrameSpellBar:GetStatusBarTexture():SetPoint('TOPLEFT', TargetFrameSpellBar, 'TOPLEFT', 0, 2)
            TargetFrameSpellBar:GetStatusBarTexture():SetPoint('BOTTOMRIGHT', TargetFrameSpellBar, 'BOTTOMRIGHT', 0, -2)
        end
        
        -- Create border texture (thinner on Y-axis)
        local border = TargetFrameSpellBar:CreateTexture('TargetCastbarBorder', 'OVERLAY')
        border:SetTexture(borderRef)
        border:SetSize(175 + 4, 8 + 4)  -- Reduced Y padding from 8 to 4
        border:SetPoint('CENTER', TargetFrameSpellBar, 'CENTER', 0, 0)
        
        -- Create spark texture
        local spark = TargetFrameSpellBar:CreateTexture('TargetCastbarSpark', 'OVERLAY')
        spark:SetTexture(sparkRef)
        spark:SetSize(20, 32)
        spark:SetPoint('CENTER', TargetFrameSpellBar, 'CENTER', 0, 0)
        spark:SetBlendMode('ADD')
        
        -- Center the spell text vertically
        ...
        -- Hook spark updates
        local originalUpdate = TargetFrameSpellBar:GetScript('OnUpdate')
        TargetFrameSpellBar:SetScript('OnUpdate', function(self, elapsed)
            if originalUpdate then
                originalUpdate(self, elapsed)
            end
            ...
            if maxVal > 0 then
                ...
                if percent >= 1 then
                    ...
                    spark:Show()
                    spark:SetPoint('CENTER', self, 'LEFT', percent * 175, 0)
                end
            end
        end)
    end
    
    -- Style focus castbar like target castbar
    if FocusFrameSpellBar then
        -- Hide all possible icon elements
        if FocusFrameSpellBar.Icon then
            FocusFrameSpellBar.Icon:Hide()
            FocusFrameSpellBar.Icon:SetAlpha(0)
        end
        if FocusFrameSpellBar.IconTexture then
            FocusFrameSpellBar.IconTexture:Hide()
            FocusFrameSpellBar.IconTexture:SetAlpha(0)
        end
        
        -- Hide all regions that might be icons
        local regions = {FocusFrameSpellBar:GetRegions()}
        for i, region in ipairs(regions) do
            if region.GetTexture and region:GetTexture() then
                local texture = region:GetTexture()
                if texture and (string.find(texture, "icon") or string.find(texture, "Icon") or region:GetWidth() == region:GetHeight()) then
        ...
        -- Apply DragonflightUI textures to focus castbar
        local standardRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarStandard2'
        local borderRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarFrame2'
        local backgroundRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarBackground2'
        local sparkRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarSpark'
        
        -- Hide original focus castbar regions to avoid double frames
        local originalRegions = {FocusFrameSpellBar:GetRegions()}
        for i, region in ipairs(originalRegions) do
            if region.GetTexture and region:GetTexture() then
                region:Hide()
                region:SetAlpha(0)
        ...
        -- Resize the focus castbar (25% smaller than target)
        FocusFrameSpellBar:SetSize(131, 6)
        
        -- Move the entire bar 25px to the left (20px + 5px)
        FocusFrameSpellBar:ClearAllPoints()
        FocusFrameSpellBar:SetPoint('TOPLEFT', FocusFrame, 'BOTTOMLEFT', 25 - 25, -5)
        
        -- Create background texture (grey texture under the filling one)
        local bgTex = FocusFrameSpellBar:CreateTexture('FocusCastbarBackground', 'BACKGROUND')
        bgTex:SetAllPoints()
        bgTex:SetTexture(backgroundRef)
        
        -- Set the status bar texture and make it visible
        if FocusFrameSpellBar:GetStatusBarTexture() then
            FocusFrameSpellBar:GetStatusBarTexture():SetTexture(standardRef)
            FocusFrameSpellBar:GetStatusBarTexture():SetAlpha(1)
            FocusFrameSpellBar:GetStatusBarTexture():Show()
            -- Make the progress bar wider on Y-axis to fill gaps
            FocusFrameSpellBar:GetStatusBarTexture():SetPoint('TOPLEFT', FocusFrameSpellBar, 'TOPLEFT', 0, 2)
            FocusFrameSpellBar:GetStatusBarTexture():SetPoint('BOTTOMRIGHT', FocusFrameSpellBar, 'BOTTOMRIGHT', 0, -2)
        end
        
        -- Create border texture (thinner on Y-axis)
        local border = FocusFrameSpellBar:CreateTexture('FocusCastbarBorder', 'OVERLAY')
        border:SetTexture(borderRef)
        border:SetSize(131 + 4, 6 + 4)  -- 25% smaller than target castbar
        border:SetPoint('CENTER', FocusFrameSpellBar, 'CENTER', 0, 0)
        
        -- Create spark texture
        local spark = FocusFrameSpellBar:CreateTexture('FocusCastbarSpark', 'OVERLAY')
        spark:SetTexture(sparkRef)
        spark:SetSize(20, 32)
        spark:SetPoint('CENTER', FocusFrameSpellBar, 'CENTER', 0, 0)
        spark:SetBlendMode('ADD')
        
        -- Center the spell text vertically
        if FocusFrameSpellBar.Text then
            FocusFrameSpellBar.Text:ClearAllPoints()
            FocusFrameSpellBar.Text:SetPoint('CENTER', FocusFrameSpellBar, 'CENTER', 0, 0)
        end
        
        -- Hook spark updates
        local originalUpdate = FocusFrameSpellBar:GetScript('OnUpdate')
        FocusFrameSpellBar:SetScript('OnUpdate', function(self, elapsed)
            if originalUpdate then
                originalUpdate(self, elapsed)
            end
            ...
            if maxVal > 0 then
                ...
                if percent >= 1 then
                    ...
                    spark:Show()
                    spark:SetPoint('CENTER', self, 'LEFT', percent * 131, 0)
                end
            end
        end)
    end
...
function Module.CreateNewCastbar()
	...
	-- Create spell icon frame above the castbar
              -- Parameters
            local iconSize = 30
           local BORDER_SCALE = 1.1  -- Adjust this to control border thickness (1.0 = same size as icon)
           
           -- Spell icon frame
           local spellIcon = CreateFrame('Frame', nil, f)
           spellIcon:SetSize(iconSize, iconSize)
           spellIcon:SetPoint('BOTTOM', f, 'TOP', 0, 8)
           
           -- Border texture (slightly bigger than icon)
           local borderTexture = spellIcon:CreateTexture(nil, 'OVERLAY')
           -- local borderSize = iconSize * borderScale
		   local width = spellIcon:GetWidth() * BORDER_SCALE
           local height = spellIcon:GetHeight() * BORDER_SCALE
           borderTexture:SetSize(width, height+1)
           borderTexture:SetPoint('CENTER', spellIcon, 'CENTER')
           borderTexture:SetTexture("Interface\\AddOns\\DrakeishUI\\assets\\uiactionbariconframe.tga") -- your border texture path
           
           -- Icon texture (centered, original size)
           local iconTexture = spellIcon:CreateTexture(nil, 'ARTWORK')
           iconTexture:SetSize(iconSize, iconSize)
           iconTexture:SetPoint('CENTER', spellIcon, 'CENTER')
           iconTexture:SetTexCoord(0.07, 0.93, 0.07, 0.93)
           
           spellIcon.Border = borderTexture
           spellIcon.Icon = iconTexture
    ...
    bg:SetPoint('TOP', f, 'BOTTOM', 0, -2)

    local bgTex = bg:CreateTexture('ImprovedUICastbarNameBackground', 'BACKGROUND')
    bgTex:ClearAllPoints()
    bgTex:SetTexture('Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\MinimapBorder')
    bgTex:SetSize(sizeX, 30)
    bgTex:SetPoint('TOP', f, 'BOTTOM', 0, -2)
    bgTex:Hide()
    ...
    -- Update spell icon texture
       local _, _, _, texture = UnitCastingInfo('player')
       if not texture then
           _, _, _, texture = UnitChannelInfo('player')
       end
	   
       if texture then
           frame.Castbar.SpellIcon.Icon:SetTexture(texture)
...
function Module.SetBarChannel()
    local channelRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarChannel'
...
function Module.SetBarInterrupted()
    local interruptedRef = 'Interface\\AddOns\\DrakeishUI\\modules\\DragonflightUI\\Textures\\Castbar\\CastingBarInterrupted2'

...
function anchor:petbar_update()
	...
	if leftbar and rightbar then
		-- self:SetPoint('TOPLEFT', pUiMainBar,'TOPLEFT', offsetX, leftOffset); original
		self:SetPoint('TOPLEFT', pUiMainBar,'TOPLEFT', offsetX, leftOffset - 14); -- without - 14
	elseif leftbar then
		self:SetPoint("TOPLEFT", pUiMainBar,'TOPLEFT', offsetX, rightOffset - 6); -- without - 6
	elseif rightbar then
		self:SetPoint("TOPLEFT", pUiMainBar,'TOPLEFT', offsetX, leftOffset - 14); -- without - 14
	else
		self:SetPoint("TOPLEFT", pUiMainBar,'TOPLEFT', offsetX, nobar - 6); -- without - 7
	end
end
...
petbar:SetScale(0.8,0.8) -- original size (1,1)
